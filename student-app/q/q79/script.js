(function (cp) {
  ...
  cp.generateNewQuestion = () => {
    ...
    let basequest = new question(
      "The reaction:<br>",
      "<br>has the K shown. If a reaction mixture has the concentrations as shown, in what direction is the reaction proceeding?<br>"
    );
    //begin programs
    function begin() {
      ...
      let quest = `<span>${basequest["p1"]}${reaction[which]}</span><span class="left-margin">K = ${K}</span>${basequest["p2"]}<hr>${series}`;
      // let quest =
      //   basequest["p1"] +
      //   reaction[which] +
      //   "  K=" +
    ...
    function display(quest) {
      ...
      // newone.document.writeln(quest);
      // newone.document.writeln("<form method='post'>");
      // newone.document.writeln(
      //   "<input type=radio name='direction' onClick='opener.answer(1)'>Right to Left"
      // );
      // newone.document.writeln(
      //   "<input type=radio name='direction' onClick='opener.answer(2)'>Left to Right"
      // );
