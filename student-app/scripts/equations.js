(function (cp) {
  const equations = [
    ...
    },
    {
      ...
      right: [{ formula: "AlCl3", coefficient: 2 }],
    },
    {
      left: [
        { formula: "P4", coefficient: 1 },
  ...
  const findFactor = (c, baseMass) => {
    c.factor =
      (cp.compoundUtils.getCompoundMass(c.formula) * c.coefficient) / baseMass;
    return c;
  };
  ...
  cp.equations = {
    getEquation: (idx) => {
      ...
      const baseMass =
        ...
        cp.compoundUtils.getCompoundMass(equation.left[0].formula);
    ...
    getRandomEquation: () => {
      const idx = cp.Random.int(0, equations.length - 1);
      return cp.equations.getEquation(idx);
    },

    getRandomEquationWithFourReagents: () => {
      ...
      return cp.equations.getEquation(
        validIdx[cp.Random.int(0, validIdx.length - 1)]
      );
    },

    getEquationHtml: (equation) => {
      return `${cp.compoundUtils.getHtmlWithCoefficient(
        equation.left[0].coefficient,
